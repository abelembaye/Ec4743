...
if "username" not in st.session_state or st.session_state.username == '' or st.session_state.username == None:
    ...
    st.title("Econ 4743 Problem Set 3")

    conn = st.connection('mysql', type='sql', ttl=60)
    query = f"SELECT * from ec4743_pset03 WHERE username = '{st.session_state.username}';"
    ...
    hw_column_types = [253, 253, 4, 4, 253, 253, 4, 252,
                       4, 252, 253, 253, 4, 4, 253, 253, 4, 253, 253]
    ...
    with st.form(key="fields_form2"):
        ...
        if submit_button:
            ...
            with conn.session as s:
                sql = text('UPDATE ec4743_pset03 SET ' + ', '.join(
    ...
    template = env.get_template("template-Econ4743-ps03.html")
